...
export function fetchOrganizationFilePage(data: PAGINATION_QUERY.Param) {
  return request<
    RESULT_SUCCESS<
      PAGINATION_QUERY.Result<FileManageOrganizationFileItem> & {
        parentPath?: { name: string; id: number; parentId: number }[];
      }
    >
...
export function fetchOrganizationCatalogTree() {
  return request<RESULT_SUCCESS<FileManageCatalogItem[]>>(
    '/file-manage/organization/catalog/tree',
    {
      method: 'GET',
    },
  );
...
export function fetchOrganizationFileToAuthGroup(fileId: number) {
  return request<RESULT_SUCCESS<FileManageFileAuthGroup[]>>(
    '/file-manage/organization/auth-group/' + fileId,
    {
      method: 'GET',
    },
  );
...
export function fetchMyFilePage(data: PAGINATION_QUERY.Param) {
  return request<
    RESULT_SUCCESS<
      PAGINATION_QUERY.Result<FileManageFileItem> & {
        parentPath?: { name: string; id: number; parentId: number }[];
      }
    >
 ...
 * 批量移动文件或目录到文件夹中
 */
export function moveManyFileOrCatalog(
  fileIds: number[],
  toCatalogId: number,
 ...
 * 批量复制文件或目录到文件夹中
 */
export function copyManyFileOrCatalog(
  fileIds: number[],
  toCatalogId: number,
  toCatalogMode: FileManageModeEnum,
) {
...
export function getOssFileIdToTimeLink(fileId: number) {
  return request<RESULT_SUCCESS<{ url: string; filename: string }>>(
    '/file-manage/oss/file-id/temp-link',
    {
      method: 'GET',
      params: {
        fileId,
