...
export function fetchRemoveOneParent(id: number) {
  return request<RESULT_SUCCESS<any>>(
    '/size-template/parent/remove/' + id,
    {
      method: 'Post',
    },
  );
...
/**@name 尺码分页 */
export function fetchSizeTemplateItemList(data: PAGINATION_QUERY.Param<SizeTemplateItemPageParamQuery>) {
  return request<RESULT_SUCCESS<PAGINATION_QUERY.Result<BusSizeTemplateItemType>>>("/size-template/item/page", {
    method: "POST",
    data
  })
}
...
export function fetchNameListSizeTemplateParent() {
  return request<RESULT_SUCCESS<(BusSizeTemplateParentType & { id: number })[]>>("/size-template/parent/all/name-to-id", {
    method: "GET",
  })
}
/**@name 获取全部尺码模板列表(仅name与id) */
export function fetchNameListSizeTemplateItem(selectId) {
  return request<RESULT_SUCCESS<{ name: string, id: number }[]>>("/size-template/item/all/name-to-id/" + selectId, {
    method: "GET",
  })
}
