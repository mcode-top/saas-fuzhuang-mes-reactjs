...
/**@name 获取自定义表分页 */
export function fetchCustomTableList(businessKey: string, data: {
  page?: number,
  limit?: number
  dataParam?: { query: any, order: any },
  basicParam?: { order: any, query: any }
}) {
  return request<RESULT_SUCCESS<PAGINATION_QUERY.Result<CustomTableDataItem>>>("/custom-form/paginate/" + businessKey, {
    method: "POST",
    data
  })
}
