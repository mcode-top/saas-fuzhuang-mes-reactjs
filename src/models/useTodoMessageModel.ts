...
import { messageCurrentMyMessage, messageCurrentSystemMessage, messageFindOneById, messageSettingRead } from "@/apis/message";
...
export default () => {
  ...
  const [systemMessage, setSystemMessage] = useState<MessageBox<MessageSystemType>>({ total: 0, list: [] });
  ...
  function addMessage(messageId: number, type: MessageTypeEnum): Promise<MessageType | MessageSystemType> {
  ...
  function readMessage(messageId: number, type: MessageTypeEnum) {
    return messageSettingRead(messageId, type).then(res => {
      if (type === MessageTypeEnum.Person) {
        ...
        syncValue.systemMessage.total -= 1;
        syncValue.systemMessage.list = syncValue.systemMessage.list.filter(m => m.id !== messageId)
        setSystemMessage({ ...syncValue.systemMessage });
